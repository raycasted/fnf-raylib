...
int storageBlammed = LoadStorageValue(STORAGE_POSITION_BLAMMED);
int storageStress = LoadStorageValue(STORAGE_POSITION_STRESS);
...
void songChoices(Font fnfFont, Color songListColors[]){
	...
	std::string blammedText = (std::string("BLAMMED ") + std::to_string(storageBlammed)).c_str();
	std::string stressText = (std::string("STRESS ") + std::to_string(storageStress)).c_str();
	//std::string YourText = (std::string("YOUR SONG") + std::to_string(LoadStorageValue(STORAGE_POSITION_YOURSONG))).c_str();

    DrawTextEx(fnfFont, blammedText.c_str(), {32, (720/2)/2}, 24, 0.0f, songListColors[0]);
    DrawTextEx(fnfFont, stressText.c_str(), {32, (720/2+48)/2}, 24, 0.0f, songListColors[1]);
