...
#include "worship.hpp"
=======
>>>>>>> 38f80d9d3d959d4afb9630bde6570014141f36fe
...
<<<<<<< HEAD
	std::string worshipText = (std::string("WORSHIP ") + std::to_string(LoadStorageValue(STORAGE_POSITION_WORSHIP))).c_str();
    ...
    DrawTextEx(fnfFont, worshipText.c_str(), {64, 720/2+96}, 48, 0.0f, songListColors[2]);
=======
...
void songDraw(std::string songName, Camera2D* camera, Rectangle* playerRec, Rectangle* enemyRec){
    if(songName == "TestSong"){
		...
		worshipDraw(playerRec, enemyRec, camera);
	}
}
void songLoad(std::string songName, std::vector<Arrow>* playerArrowList, std::vector<Arrow>* computerArrowList, Music* instrumental, Music* player, Rectangle* playerRec, Rectangle* enemyRec, Camera2D* camera, int difficulty){
    if(songName == "TestSong"){
		...
		worship(playerArrowList, computerArrowList, instrumental, player, playerRec, enemyRec, camera, difficulty);
	}
}
void songSave(std::string songName, int score){
	...
	else if(songName == "Worship" && score > LoadStorageValue(STORAGE_POSITION_WORSHIP)){
		SaveStorageValue(STORAGE_POSITION_WORSHIP, score);
	}
}